...
- Allow `--answers` option for `underware configure local` command (https://github.com/alces-software/underware/pull/447)

## [2018.3.0] - 2018-08-15

- Moved asset types to be stored in Underware itself rather than the repo being
  used (https://github.com/alces-software/underware/pull/394).
- Store assets by type (https://github.com/alces-software/underware/pull/393).
- Provide access to assets by type in namespace
  (https://github.com/alces-software/underware/pull/401).
- Added `layout add` and `layout edit` to create and edit asset layouts
  (https://github.com/alces-software/underware/pull/400,
  https://github.com/alces-software/underware/pull/406)
- Added method to nodes in namespace to check if node is the local node
  (https://github.com/alces-software/underware/pull/409).
- Added ability to save or edit sub-assets when creating an asset
  (https://github.com/alces-software/underware/pull/414).
- Added ability to create an asset from a layout as well as a type
  (https://github.com/alces-software/underware/pull/415).
- Added more data to asset types
  (https://github.com/alces-software/underware/pull/420).
- Switched `named` and `hosts` files to be rendered as Underware-managed files
  (https://github.com/alces-software/underware/pull/418).
- Fixed messages not being printed until build script finished in local build
  (https://github.com/alces-software/underware/pull/419).
- Added `eval` command for programmatic access to Underware namespace
  (https://github.com/alces-software/underware/pull/435).
- Fixed non-Kickstart build methods often not being detected
  (https://github.com/alces-software/underware/pull/440).

## [2018.2.0] - 2018-04-25

- Fixed default values not being displayed when configuring when question
  `type` is `integer` (https://github.com/alces-software/underware/pull/357).
- Added `asset` commands for managing asset files with the following
  ...
  - `asset add` which creates a new asset from an asset type template in repo
    (https://github.com/alces-software/underware/pull/364).
  - `asset edit` which opens existing assets in the system editor
    (https://github.com/alces-software/underware/pull/365).
  - `asset link/unlink` which manage the relationships between assets and nodes
    (https://github.com/alces-software/underware/pull/373).
  - `asset delete` which deletes an asset and unassigns it from the nodes.
- Made assets configured using the above available within Underware namespace
  (https://github.com/alces-software/underware/pull/362).

## [2018.1.0] - 2018-03-27

- Added support for conditional Underware configuration questions.
...
- Added `plugin` commands for Underware plugin management.
- Added support for configuration of Underware plugins.
...
- Improved commands which take `-g`/`--group` option so they can operate on any
  ...
  (https://github.com/alces-software/underware/pull/306).
- Fixed `view-answers` commands to work with latest `configure.yaml` format
  (https://github.com/alces-software/underware/pull/311).
- Suppressed output suggesting there might be a problem with Underware when
  displaying known user input errors
  (https://github.com/alces-software/underware/pull/314).
- Added `overview` command to give overview of state of whole domain and
  configured groups (https://github.com/alces-software/underware/issues/284).
...
- Fixed various issues when `configure`ing where answers incorrectly
  ...
  defaults (https://github.com/alces-software/underware/issues/330).
- Fixed inconsistent and undocumented overriding of defaults at different
  levels (https://github.com/alces-software/underware/pull/335).
- Fixed `power` and `ipmi` commands run on groups to not fail, and just report
  ...
  fail (https://github.com/alces-software/underware/issues/337).
- Fixed issue where orphan nodes would be saved in the group cache even if the
  ...
  (https://github.com/alces-software/underware/issues/341).
- Fixed issue with defaults not being set for non-domain level questions
  (https://github.com/alces-software/underware/issues/345).
- Support various options to `ipmi` command that were accepted in previous
  versions of Underware (`--command`/`-c`/`-k`)
  (https://github.com/alces-software/underware/issues/348).
...
- Added `underware template` command which renders the templates and build files
  for a node or group. All files are rendered to the staging directory and not
  their final location.
- Added `underware sync` command which moves staged files in place and restarts
  associated services.
- Remove rendering of templates/ files as part of `underware build`. This has been
  replaced by `underware template` followed by `underware sync`. Also removed the
  obsolete edit start and continue flags.
- Removed the `underware dhcp` command as it has been replaced by `underware template`
  followed by `underware sync`.
- Added staging to rendering of genders file in `underware configure`. The genders
  file needs to be synced (using `underware sync`) before it can be used by `underware
  template`.
- Fixed the bugs that slowed down underware. A shared cache is now used to
  render all underware templates/ files.
...
- Added `underware configure` commands, to answer questions for various objects
  managed by Underware and save the answers for use in later templating.
- Added support for having Underware use `named` instead of `hosts` file for
  managing host name resolution.
- Added full rendering of various domain templates when `underware configure`
  commands are run, including `genders` file and `hosts`/`named` template.
- Removed `underware hosts` command, as made obsolete by above change.
...
- Added various `alces` namespace parameters including `group_index`, `groups`,
  and `answers`.
- Added `underware hunter` `--ignore-duplicate-macs` option to ignore already found
  MAC addresses.
- Added `underware view-config` command to view the complete current Underware
  config.
- Added `underware view-answers` commands to view the configured answers applicable
  to different Underware objects.
- Added `underware remove group` command to remove a configured group.
...
- Implemented prototype Underware GUI; not yet publicly exposed.
- Numerous other bug fixes and minor tweaks.
- Switched to new year-based versioning system.

## [2.0.0] - 2017-08-07

- Rewrote and restructured much of Underware to be more consistent and
  flexible.
- Only using a single Underware repo at a time is now supported, and the `repo`
...
- Arbitrary files from the repo, the Underware deployment server, or any URL,
...
- A new `render` command provides direct access to the Underware templater, to
...
- See [the initial design document](docs/design/01-underware-improvements.md)
...
- Refactored Underware content to separate Underware content repos
- Improved determination of `hostip` when templating

### Added

- Created repo system for Underware content and new `underware repo` command to
  manage these
- Added `underware status` tool to display the `ping` and `power` state of the
  nodes
...
### Changed
- Removed cobbler integration in 'underware hunter'
